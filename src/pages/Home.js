...
export const Home = props => {
    ...
    const addToCart = (product) => {
        ...
        for (let i = 0; i < cart.length; i++) {
            if (cart[i].id === productToAdd.id) {
                ...
                productToModify.quantity = cart[i].quantity + productToAdd.quantity
    ...
    if (!products.length) return <div className='prod-list' ><OrbitSpinner color='blue' size={300} /></div>
    return (
        <div>
            <ul>
                {
            ...
            <ProductList products={products} addToCart={addToCart} onProductClick={onProductClick} />
